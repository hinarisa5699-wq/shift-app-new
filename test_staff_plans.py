...
def test_cooking_patterns_9_to_16_and_9_to_19_and_7_to_13_exist(tmp_path, monkeypatch):
    """調理シフトに 9:00-16:00 / 9:00-19:00 / 7:00-13:00 の種類がある。

    ユーザー依頼 2026-08:「調理のシフト 9時から16時 項目追加」
    「調理9時から19時も追加」、2026-09:「調理7:00-13:00のシフト作成」。
    ...
    with flask_app.app_context():
        for start, end in (("09:00", "16:00"), ("09:00", "19:00"),
            ...
            assert "{}-{}".format(
                start.lstrip("0"), end.lstrip("0")) in rows[0].label
